...
public final class SMPToolkitPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            getLogger().severe("Could not initialize SQLite: " + ex.getMessage());
        ...
        // Instantiate everything before enabling it so cross-module getters are safe during enable().
        stats = new StatsModule(this);
        locator = new LocatorModule(this);
        utilities = new UtilitiesModule(this);
        gameplay = new GameplayModule(this);
        staff = new StaffModule(this);
        moderation = new ModerationModule(this);
        altCheck = new AltCheckModule(this);
        chat = new ChatControlModule(this);
        chatGames = new ChatGamesModule(this);
        ...
        enableIfConfigured(stats);
        enableIfConfigured(locator);
        enableIfConfigured(utilities);
        enableIfConfigured(gameplay);
        enableIfConfigured(staff);
        enableIfConfigured(moderation);
        enableIfConfigured(altCheck);
        enableIfConfigured(chat);
        enableIfConfigured(chatGames);
        enableIfConfigured(events);
        enableIfConfigured(announcements);

        CommandRouter router = new CommandRouter(this);
        router.register();

        getLogger().info("SMPToolkit enabled with " + enabledModules.size() + " modules: " +
                String.join(", ", enabledModules.stream().map(Module::name).toList()));
    }

    @Override
    public void onDisable() {
        ListIterator<Module> it = enabledModules.listIterator(enabledModules.size());
        while (it.hasPrevious()) {
            Module module = it.previous();
            try { module.disable(); }
            catch (Throwable ex) { getLogger().warning("Error disabling " + module.name() + ": " + ex.getMessage()); }
        }
        enabledModules.clear();
        if (database != null) database.close();
    }

    private void enableIfConfigured(Module module) {
        if (!getConfig().getBoolean("modules." + module.name(), true)) {
            getLogger().info("Module disabled by config: " + module.name());
            return;
        }
        try {
            module.enable();
            ...
            getLogger().severe("Failed to enable module " + module.name() + ": " + ex.getMessage());
            ex.printStackTrace();
        }
    }

    private void ensurePrivacySalt() {
        String salt = getConfig().getString("privacy.ip-hash-salt", "");
        if (salt != null && !salt.isBlank()) return;
        byte[] bytes = new byte[32];
        new SecureRandom().nextBytes(bytes);
        getConfig().set("privacy.ip-hash-salt", Base64.getUrlEncoder().withoutPadding().encodeToString(bytes));
        saveConfig();
    }
    ...
    public boolean isModuleEnabled(String name) {
        return enabledModules.stream().anyMatch(m -> m.name().equalsIgnoreCase(name));
    }
